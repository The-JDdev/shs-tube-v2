...
class BrowserFragment : Fragment() {
    ...
    override fun onCreateView(i: LayoutInflater, c: ViewGroup?, b: Bundle?): View {
        ...
        configureChromiumStyle(wv)
        BrowserSettings.applyToWebView(wv, requireContext())

        wv.addJavascriptInterface(
            MediaSniffer.JsBridge { currentPageUrl },
            "SHSSnifferBridge"
        )

        wv.webChromeClient = object : WebChromeClient() {
            override fun onProgressChanged(view: WebView?, newProgress: Int) {
                if (newProgress == 100) MediaSniffer.inject(wv)
            }
        ...
        v.findViewById<ImageButton>(R.id.btn_reload).setOnClickListener { wv.reload() }

        btnSettings.setOnClickListener {
            val host = try { Uri.parse(wv.url ?: "").host } catch (_: Throwable) { null }
        ...
        downloadIcon.setOnLongClickListener {
            val pageUrl = wv.url ?: urlBar.text.toString()
    ...
    private fun configureChromiumStyle(wv: WebView) {
        ...
        s.userAgentString = s.userAgentString.replace("; wv", "") + " SHSTube/2.4"
