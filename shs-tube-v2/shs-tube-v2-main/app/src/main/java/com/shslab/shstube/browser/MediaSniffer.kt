...
object MediaSniffer {
    ...
    fun reportNetworkResource(url: String?, mime: String?, sourcePage: String?) {
        ...
        // Skip small images — only sniff substantial media (ignore icons, tracking pixels, etc.)
        // Images with explicit MIME are always sniffed; extension-only matches rely on this filter.
        if (!mimeOk && ext) {
            val isImage = url.contains(Regex("""\.(jpg|jpeg|png|gif|webp)""", RegexOption.IGNORE_CASE))
            if (isImage) {
                // For images without explicit MIME, skip tiny icons/thumbnails by URL heuristics
                val lcUrl = url.lowercase()
                val isLikelySmallIcon = lcUrl.contains("/favicon") || lcUrl.contains("/icon")
                    || lcUrl.contains("/logo") || lcUrl.contains("/badge")
                    || lcUrl.contains("/sprite") || lcUrl.contains("1x1")
                    || lcUrl.contains("/pixel") || lcUrl.contains("/tracking")
                if (isLikelySmallIcon) return
            }
        }
