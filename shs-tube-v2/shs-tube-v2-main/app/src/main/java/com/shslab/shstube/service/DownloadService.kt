...
class DownloadService : Service() {
    ...
    companion object {
        ...
        private val outputPatterns = ConcurrentHashMap<Long, String>()
    ...
    private suspend fun runJob(
        ...
        // 3. Run download
        val outDir = StoragePrefs.publicDownloadDir()
        // Use a unique output template so we can reliably find the file after download
        val safeName = "shs_${rowId}_%(title)s.%(ext)s"
        val outputPath = File(outDir, safeName).absolutePath
        outputPatterns[rowId] = "shs_${rowId}_"
        ...
        try {
            ...
            if (cancelled.remove(rowId)) {
                ...
                if (finalFile != null && finalFile.exists()) {
                    // Rename to clean filename (remove the shs_ID_ prefix)
                    val cleanName = finalFile.name.removePrefix(prefix)
                    val cleanFile = if (cleanName.isNotBlank()) {
                        val dest = File(finalFile.parent, cleanName)
                        try {
                            if (dest.exists()) dest.delete()
                            if (finalFile.renameTo(dest)) dest else finalFile
                        } catch (_: Throwable) { finalFile }
                    } else finalFile
