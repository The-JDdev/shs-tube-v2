...
object AdBlocker {
    ...
    fun maybeBlock(url: String?): WebResourceResponse? {
        ...
        // Never block first-party resources or common CDN domains that host mixed content
        val lc = url.lowercase()
        val host = try {
            android.net.Uri.parse(lc).host ?: return null
        } catch (_: Throwable) { return null }

        // Skip blocking for common legitimate CDN/resource hosts that sometimes get caught
        val safeHosts = setOf("cdn", "fonts.googleapis.com", "fonts.gstatic.com",
            "cdnjs.cloudflare.com", "cdn.jsdelivr.net")
        if (host in safeHosts) return null
